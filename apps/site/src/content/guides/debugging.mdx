...
The overlay's `pegs` layer draws the pegs the tongues touch, fills the one being ridden, and marks each tongue's contact zone on the rim; the HUD prints the first tongue's deflection, and the arc the tongues are holding back when `flap.drag` is on. Tune `elasticity`, `friction`, `stiffness`, `damping` and `drag` while watching it. The [tongue recipe](/recipes/tongue/) is the long version.
