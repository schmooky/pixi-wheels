...
**Leg**: one piece of a planned stop: a distance, a duration, an ease, forwards or reverse. Kinds: decel, creep, dwell, push, return, skip, settle, bounce, bounce-return.
...
**Anticipation**: a planned near-miss. The pointer heads for the bait, lands on the target, and rests by the line the two share. `anticipation:start`, `anticipation:bait`, `anticipation:end`. [Anticipation](/guides/anticipation/)
...
**Creep**: the pointer crawls through the bait and barely crosses the line into the target.

**Overshoot**: the wheel runs out of momentum with the pointer a few degrees past the line, into the bait, holds a beat and rolls softly back onto the target. Never more than a quarter of the bait, five degrees by default.

**Rest**: where the pointer ends after a tease: a fraction of the target's arc (0.22 by default) from the divider it shares with the bait. `rest: 'keep'` leaves the landing mode in charge.

**Stutter**: the wheel halts with the pointer a hair inside the bait, holds, then nudges over the line.

**Auto**: picks creep, stutter or overshoot from the distance between bait and target.
...
**Flap**: the pointer's spring deflection on each divider crossing: `maxAngle`, `stiffness`, `damping`. Also **kick**.
...
**Tick**: a divider passing under a pointer. `pointer:tick` carries the pointer, the sections either side, the speed and the direction. Pointer skins with a `tick` animation play it here.
...
**Debug overlay**: `debugOverlay(wheel)`: section boundaries, pointer angles, the target and a HUD drawn over the wheel, live.
