...
**Flap**: the tongue's deflection against the pegs: the smallest swing that keeps the peg outside the blade (`elasticity` throws it further), held while the peg passes (`friction` holds it longer), sprung back (`stiffness`, `damping`), never past `maxAngle`. [Pointers](/guides/pointers/)
