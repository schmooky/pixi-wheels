...
**Blade**: the tongue as the contact model sees it - a triangle hinged at the pin, `tipWidth` across at its base, tapering to a point at the tip. A peg is a circle, and the two may never overlap. [The tongue](/recipes/tongue/)

**Flap**: the tongue's deflection against the pegs: the smallest swing from where it stands that keeps the peg outside the blade, held while the peg passes, sprung back (`stiffness`, `damping`), never past `maxAngle`. `elasticity` and `friction` are padding on the peg - a fatter peg swings the blade wider and holds it longer. [Pointers](/guides/pointers/)
...
**Bite**: how far a peg's shoulder reaches past the tongue's tip. `pegs.inset` defaults to `tipInset + size - 2`, a 2 px bite; pegs set out past the tip make the blade lift clear over them instead, which is a much bigger flap.
...
**Boss**: the round heel a pointer is drawn with, centred on the pin and sized to clear it. Every `GraphicsPointerSkin` shape is the hull of the boss and the tip circle, so the silhouette is one a hinge could actually hold.
...
**Reach**: whether the peg ring runs between a pointer's pin and its tip, which is the only place a peg can touch it. `pointer.reaches(pegs)`; a pointer out of reach warns once and stays still. [The tongue](/recipes/tongue/)
