...
<Figure src="/recipes/tongue/contact.svg" caption="u is the peg's position along its own rim, in px, measured against the tongue's rest axis." />

The tip is a point `tipWidth` wide sitting at the peg ring. A peg arrives, pushes the tip aside along its own rim exactly as far as the geometry demands times `elasticity`, and is carried flat once it is over the crown. Two numbers set the scale of all of it: the contact half-width `c` is `pegs.size + tipWidth / 2`, and `D` is the gap between the pin and the peg ring, so a long tongue bends through a smaller angle for the same shove. `friction` decides how much further the peg drags the tongue after it is technically through. Then it lets go, and a spring takes over.
...
The lever the peg turns is `pinRadius - pegs.radius`, so a long tongue swings through a smaller angle for the same shove. Lengthening the tongue and keeping the feel means raising `elasticity` to match.
...
`maxAngle` clamps whatever comes out of that. A tongue that hits its limit on every peg reads as rigid rather than as fast, so raise it before you raise `elasticity`.
