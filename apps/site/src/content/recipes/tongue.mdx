...
The other arrangement: studs right at the edge of the wheel and a long flapper lying across them, the way a carnival wheel is built. The contact does not change - the blade still may not touch a peg - but the way out does. A peg sitting deep under the tip cannot slip past it sideways, so the blade has to ride up until its tip lifts clear over the peg entirely.
...
```ts
.radius(215)
.pegs({ size: 11, inset: 4 })       // centres 4 px inside the rim: on the edge
.pointer({
  tipInset: 50,                     // the tip hangs below the pegs, between them
  skin: { type: 'graphics', shape: 'tongue', length: 84, width: 30 },
  flap: { maxAngle: 70, stiffness: 240, damping: 9, friction: 0.3 },
})
```

<RecipeDemo code="rim-pegs" height={440} />

`maxAngle` is the thing to get right. Leave it at the default 45 here and the blade cannot clear a peg at all, so it rides through them - the engine says so once and tells you the angle this geometry actually needs. The readout above shows the largest lift of the last spin; set the cap a little above it.
